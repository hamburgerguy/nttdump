...
def newfuckery(par,f):
    if isinstance(f,list):
        mp = f
    else:
        mp = f.tolist()
    tmp = [i for i in mp]
    h1 = tmp[0:(len(tmp) // 2)-1]
    h2 = tmp[(len(tmp)// 2)-1:len(tmp)]
    if tmp[-1] != 0:
        h2.append(0)
    a = array(h1,longlong)
    b = array(h2,longlong)

    fin = [central(par,(x-y)) for x,y in zip(a,b)]
...
def fuckery(par,f):
    ...
    a = array(h1, longlong)
    b = array(h2, longlong)
    combo = [central(par,(x-y)) for x,y in zip(a,b)]

    return combo


simp = newfuckery(par,npmod)
sump = newfuckery(par,nppol)
somp = newfuckery(par,nut)

stump = [central(par,x) for x in nut]
mump = [central(par,x) for x in npmod]
gump = [central(par,x) for x in nppol]

mod = newfuckery(par,mump)
unmod = newfuckery(par,gump)
nt = newfuckery(par,stump)


print(sum(stump))
print(sum(mump))
print(sum(gump))
print('ntt: '+str(sum(nut)))
print('npmod: '+str(sum(npmod)))
print(sum(nut)-sum(npmod))
print('central pre-applied: ____________')
print('fuckery modded: ' + str(sum(mod))+' fuckery unmodded: '+str(sum(unmod))+' fuckery ntt '+str(sum(nt)))
print(sum(fuckery(par,npmod)))
print(sum(fin))
print(sum(fuckery(par,nut)))
print(sum(fuckery(par,stump)))
print(sum(fuckery(par,nppol)))
print(sum(fuckery(par,oldpoly)))
print('what I want it to be: '+str(sum(pol)))
print(abs(sum(somp)-sum(pol)))
print(sum(somp))
print(sum(oldpoly))
print(sum(nppol))
print(sum([i%par['modulus'] for i in oldpoly]))
...
gx = [1,6,1,0]
q = 7
mult = polymulmod(fx,gx,4,4,q)

palmul = poly_mul({'modulus': q},fx,gx)
print(mult)
print(palmul)
h1 = mult[0:len(mult)//2]
h2 = mult[len(mult)//2:len(mult)]
print(h1)
print(h2)
h12 = [(x-y) for x,y in zip(h1,h2)]
print(h12)
fin = [central({'modulus': 7}, _) for _ in h12]
print(fin)
'''
...
test1 = [2,0,0,1]
test2 = [1,6,1,0]
print(polymulmod(test1,test2,len(test1),len(test2),{'modulus':7}))
newb = polymul(test1,test2).tolist()
print(fuckery({'modulus':7},newb))
'''
gah = polymul(f,g)
aahh = gah.tolist()
cum = fuckery(par,aahh)
yes = array(cum)
tit = yes.T
po = array(pol)
pot = po.T
...
for i in range(len(tit)):
    if tit[i] != pot[i]:
        ...
        print(tit[i])
        print(pot[i])
        print(abs(tit[i]- pot[i]))
        if (abs(tit[i]-pot[i])) == 419512324:
            val += 1
        else:
            val2 += 1
            others.append((abs(tit[i]-pot[i])))
...
print(nut)
for i in nut:
    store += bin(i)[2:].zfill(32)
    lstt.append(bin(i)[2:].zfill(32))

sto = bin_dig_to_poly(par,lstt,1)
#print(len(sto))


trying = [central(par,x,(y*(par['modulus']))+((par['modulus']-1)//2)) for x,y in zip(nut,sto)]
print(sum(trying))
print(sum(stump))
print(sum(fuckery(par,trying)))
print(sum(mump))
print(sum(gump))
print(sum(pol))

inpol = intt(npmod,par,inverse=True)
print('F^-1(f*g): ' + str(sum(inpol)))
whateva = [abs(x-y) for x,y in zip(pol,fuckery(par,nut))]
print(whateva)
print(len(whateva))
print(len(pol))
print(fuckery(par,nut)[-1])
print(pol[-1])